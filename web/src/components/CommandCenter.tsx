...
function IntegrationDialog({ integration, busy, onClose, onSave }: { integration: IntegrationStatus; busy: boolean; onClose: () => void; onSave: (request: { name: string; baseUrl?: string | null; enabled: boolean; secret?: string | null; settings?: Record<string, string> }) => Promise<void> }) {
  return <div className="command-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><form className="command-modal integration-modal" onSubmit={(event) => { event.preventDefault(); const data = new FormData(event.currentTarget); const settings: Record<string, string> = {}; ["topic", "model", "roots", "prefix", "allowedUserIds", "allowedChannelIds", "allowedGuildIds"].forEach((key) => { const value = String(data.get(key) ?? ""); if (value) settings[key] = value; }); void onSave({ name: String(data.get("name")), baseUrl: String(data.get("baseUrl")) || null, enabled: data.get("enabled") === "on", secret: String(data.get("secret")) || null, settings }); }}><header><div><span className="section-kicker">Integration</span><h3>{integration.name}</h3></div><button type="button" onClick={onClose}><X size={17} /></button></header><label className="switch-row"><input name="enabled" type="checkbox" defaultChecked={integration.enabled} /><span>Enabled</span></label><label><span>Name</span><input name="name" defaultValue={integration.name} required /></label>{integration.id !== "discord" ? <label><span>Endpoint</span><input name="baseUrl" type="url" defaultValue={integration.baseUrl ?? ""} placeholder="https://..." /></label> : null}<label><span>{integration.hasSecret ? "Replace credential" : integration.id === "discord" ? "Bot token" : "Credential"}</span><input name="secret" type="password" autoComplete="new-password" placeholder={integration.hasSecret ? "Leave blank to keep saved value" : integration.id === "discord" ? "Discord bot token" : "Token or API key"} /></label>{integration.id === "ntfy" ? <label><span>Topic</span><input name="topic" placeholder="homedashboard" /></label> : null}{integration.id === "ollama" ? <label><span>Model</span><input name="model" placeholder="qwen3:4b" /></label> : null}{integration.id === "windows" ? <label><span>Allowed roots</span><input name="roots" placeholder="D:\\Media; E:\\Backups" /></label> : null}{integration.id === "discord" ? <><label><span>Command prefix</span><input name="prefix" placeholder="!hd" /></label><label><span>Allowed user IDs</span><input name="allowedUserIds" inputMode="numeric" placeholder="Required · comma-separated" /></label><label><span>Allowed channel IDs</span><input name="allowedChannelIds" inputMode="numeric" placeholder="Optional · comma-separated" /></label><label><span>Allowed server IDs</span><input name="allowedGuildIds" inputMode="numeric" placeholder="Optional · comma-separated" /></label></> : null}<div className="capability-chips">{integration.capabilities.map((item) => <span key={item}>{item}</span>)}</div><footer><button type="button" onClick={onClose}>Cancel</button><button className="primary" type="submit" disabled={busy}><Check size={15} />Save</button></footer></form></div>;
}
